...
from nexus_core.utils.exceptions import ManagerInitializationError, APIError
...
@pytest.fixture
def api_config():
    return {'enabled': True, 'host': '127.0.0.1', 'port': 8000, 'workers': 4, 'cors': {'origins': ['*'], 'methods': ['*'], 'headers': ['*']}, 'rate_limit': {'enabled': False, 'requests_per_minute': 100}}
@pytest.fixture
...
def api_manager_dependencies():
    ...
    security_manager.refresh_token.return_value = {'access_token': 'new_access_token', 'token_type': 'bearer', 'expires_in': 1800}
    ...
    registry = {'app_core': MagicMock(), 'config': config_manager_mock, 'security': security_manager, 'plugin_manager': MagicMock(), 'monitoring': MagicMock()}
    registry['app_core'].status.return_value = {'name': 'ApplicationCore', 'initialized': True, 'healthy': True, 'version': '0.1.0', 'managers': {}}
    return (logger_manager, security_manager, event_bus_manager, thread_manager, registry)
@pytest.fixture
def api_manager(config_manager_mock, api_manager_dependencies):
    logger_manager, security_manager, event_bus_manager, thread_manager, registry = api_manager_dependencies
    with patch('nexus_core.core.api_manager.FastAPI'), patch('nexus_core.core.api_manager.APIRouter'), patch('nexus_core.core.api_manager.OAuth2PasswordBearer'), patch('nexus_core.core.api_manager.CORSMiddleware'), patch('fastapi.Depends'), patch.object(APIManager, '_start_api_server'):
        api_mgr = APIManager(config_manager_mock, logger_manager, security_manager, event_bus_manager, thread_manager, registry)
        api_mgr.initialize()
        yield api_mgr
        api_mgr.shutdown()
def test_api_manager_initialization(config_manager_mock, api_manager_dependencies):
    logger_manager, security_manager, event_bus_manager, thread_manager, registry = api_manager_dependencies
    with patch('nexus_core.core.api_manager.FastAPI') as mock_fastapi, patch('nexus_core.core.api_manager.APIRouter') as mock_router, patch('nexus_core.core.api_manager.OAuth2PasswordBearer'), patch('nexus_core.core.api_manager.CORSMiddleware'), patch.object(APIManager, '_start_api_server'):
        mock_app = MagicMock()
        ...
        event_bus_manager.publish.assert_called_with(event_type='api/started', source='api_manager', payload={'host': '127.0.0.1', 'port': 8000, 'url': 'http://127.0.0.1:8000/api'})
        api_mgr.shutdown()
...
def test_api_manager_disabled(config_manager_mock, api_manager_dependencies):
    ...
    api_mgr.shutdown()
def test_api_manager_initialization_failure(config_manager_mock, api_manager_dependencies):
    logger_manager, security_manager, event_bus_manager, thread_manager, registry = api_manager_dependencies
    with patch('nexus_core.core.api_manager.fastapi', None):
        api_mgr = APIManager(config_manager_mock, logger_manager, security_manager, event_bus_manager, thread_manager, registry)
...
def test_register_api_endpoint(api_manager):
    ...
    result = api_manager.register_api_endpoint(path='/test', method='get', endpoint=test_endpoint, tags=['Test'], summary='Test endpoint')
...
def test_on_config_changed(api_manager):
    ...
    api_manager._logger.warning.assert_called_with('Changing api.enabled requires restart to take effect', extra={'enabled': False})
    api_manager._on_config_changed('api.port', 8080)
    api_manager._logger.warning.assert_called_with('Changing api.port requires restart to take effect', extra={'port': 8080})
    api_manager._on_config_changed('api.cors.origins', ['http://localhost:3000'])
    api_manager._logger.warning.assert_called_with('Changing api.cors.origins requires restart to take effect', extra={'origins': ['http://localhost:3000']})
@pytest.mark.asyncio
async def test_endpoint_handlers(api_manager):
    ...
    async def test_login():
        user_data = security_manager.authenticate_user(form_data.username, form_data.password)
        if not user_data:
            return None
