...
def test_event_creation():
    ...
    event = Event(event_type='test/custom', event_id=custom_id, timestamp=custom_time, source='custom_source', payload=custom_payload, correlation_id=custom_correlation)
...
def test_event_factory_method():
    event = Event.create(event_type='test/factory', source='factory_source', payload={'factory': True}, correlation_id='correlation123')
...
def test_event_subscription_creation():
    callback = MagicMock()
    subscription = EventSubscription(subscriber_id='test-subscriber', event_type='test/event', callback=callback)
    ...
    filtered_subscription = EventSubscription(subscriber_id='filtered-subscriber', event_type='filtered/event', callback=callback, filter_criteria=filter_criteria)
...
def test_event_subscription_matching():
    ...
    filtered_subscription = EventSubscription(subscriber_id='filtered-subscriber', event_type='filtered/event', callback=callback, filter_criteria={'category': 'important', 'priority': 'high'})
    matching_filtered_event = Event(event_type='filtered/event', source='test_source', payload={'category': 'important', 'priority': 'high', 'other': 'value'})
    partially_matching_event = Event(event_type='filtered/event', source='test_source', payload={'category': 'important', 'priority': 'low'})
    non_matching_filtered_event = Event(event_type='filtered/event', source='test_source', payload={'category': 'normal', 'other': 'value'})
    ...
    wildcard_subscription = EventSubscription(subscriber_id='wildcard-subscriber', event_type='*', callback=callback)
    any_event = Event(event_type='any/type', source='any_source')
    assert wildcard_subscription.matches_event(any_event) is True
    wildcard_filtered_subscription = EventSubscription(subscriber_id='wildcard-filtered', event_type='*', callback=callback, filter_criteria={'important': True})
    important_event = Event(event_type='any/important', source='any_source', payload={'important': True})
    unimportant_event = Event(event_type='any/unimportant', source='any_source', payload={'important': False})
...
def test_event_subscription_callback():
    callback = MagicMock()
    subscription = EventSubscription(subscriber_id='test-subscriber', event_type='test/event', callback=callback)
    event = Event(event_type='test/event', source='test_source', payload={'data': 'test'})
